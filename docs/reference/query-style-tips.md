...
> 💡 이 페이지의 예시는 **모양(형식)**을 보여 주기 위해 테이블 주소를 `transactions`처럼 짧게 줄였습니다. 실제로 실행할 때는 `` `bdai13-bigquery.tabformer.transactions` ``처럼 전체 주소를 쓰고, 문자열인 `amount`는 [숫자로 바꾼 뒤](bigquery-syntax.md#5-타입-변환과-정제) 더하세요.
...
LEFT JOIN cards AS card
  ON customer.user_id = card.user   -- cards 테이블의 고객 번호 컬럼 이름은 user
```
...
SELECT
  ...
  COUNT(*) AS transaction_count
FROM transactions
-- 온라인 거래는 merchant_city가 'ONLINE'이라 지역 분석에서 제외
