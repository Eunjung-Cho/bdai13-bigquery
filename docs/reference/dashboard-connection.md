...
기본 경로는 **BigQuery 집계 마트를 Python으로 조회해 Streamlit에 표시**하는 것입니다. CSV로 내보내지 않습니다. 인증 정책 때문에 앱에서 직접 연결하기 어려우면 Colab에서 로그인해 DuckDB 스냅샷을 만드는 예비 경로를 씁니다.
...
```mermaid
flowchart LR
    R["강사 원천<br/>finda-13-2026.tabformer"] --> S[학생 SQL 분석]
    S --> M["학생 집계 마트<br/>월 × MCC × 채널"]
    M --> P[Python BigQuery 클라이언트]
    P --> A[Streamlit 앱]
```

앱은 2,400만 원천 거래를 메모리로 가져오지 않습니다. 5주차에서 만든 작고 검증 가능한 마트만 읽습니다. 원천에서 마트를 만드는 비용과 앱에서 마트를 읽는 비용도 구분합니다.
...
공개 기본 앱에는 여기에 매개변수·행 수 제한·캐시를 추가해 두었습니다. `maximum_bytes_billed`는 1회 쿼리 상한이고, `LIMIT`는 반환 행 제한입니다. 역할이 다릅니다.
...
**원천 전체 읽기 권한이나 프로젝트 Owner를 앱에 부여하지 않습니다.** 이 앱은 저장된 집계 테이블을 읽으므로 해당 테이블에 필요한 조회 권한만 줍니다. 한 서비스 계정 키를 반 전체에 공용으로 배포하지 않고 학생·팀별 구성을 사용합니다. 교사는 화면 순서 안내와 템플릿을 제공하고 실제 키는 각자 Secrets에 입력합니다.

조직 정책으로 서비스 계정 키 생성이 차단되어 있으면 정책을 끄는 실습을 하지 않습니다. Colab에서 본인 계정의 조회가 가능하면 아래 DuckDB 경로를 이용합니다. 본인 계정도 원천·마트 조회 권한이 없다면 먼저 그 권한부터 해결해야 합니다.
...
위 설정 다음에 Google에서 발급한 서비스 계정 JSON의 필드를 `[gcp_service_account]` 블록으로 옮깁니다. 줄바꿈이 포함된 private_key는 예제 양식에 맞춰 입력합니다. **실제 키를 GitHub에 올리거나 AI에게 보내지 않습니다.** 프로젝트 ID·테이블 ID·리전만 AI에 알려 주면 코드 수정에 충분합니다.

5주차 마트의 필수 컬럼은 `tx_month`, `mcc`, `channel`, `amount_usd`, `txn_count`, `fraud_count`, `fraud_labeled_count`입니다. 이름을 바꾸면 코드도 함께 수정해야 합니다. `YOUR_PROJECT`는 학생마다 달라지며 강사 원천 ID를 무조건 넣는 칸이 아닙니다.
...
GitHub Pages는 정적 강의 웹을 호스팅합니다. Python Streamlit 앱은 별도의 Community Cloud URL에서 실행하며 강의 웹에서 그 URL을 연결합니다.
...
DuckDB는 파일 하나로 사용할 수 있는 분석용 데이터베이스입니다. 제공 노트북을 Colab에 업로드하고 다음 순서로 실행합니다.
...
7. 공개 가능한 파일인지 확인하고 앱 저장소 루트에 추가.
...
**수동 CSV 단계는 없습니다.** 다만 DuckDB 파일은 저장 시점의 스냅샷입니다. 새 데이터를 반영하려면 노트북을 다시 실행해 파일을 교체합니다. Colab을 상시 웹서버로 열거나 외부 터널로 Streamlit을 호스팅하는 방식은 사용하지 않습니다. 노트북은 데이터 준비, Community Cloud는 앱 공개를 담당합니다.

DuckDB는 BigQuery의 조회 권한이나 무료 쿼리 한도를 없애 주지 않습니다. BigQuery 자체 조회가 실패하면 원천 파일을 별도로 확보해야 하며, 존재하지 않는 데이터를 DuckDB로 연결할 수는 없습니다.
