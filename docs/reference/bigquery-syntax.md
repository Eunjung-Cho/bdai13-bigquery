...
1~3주차 실습에서 쓰는 문법을 한 장에 모았습니다.
**여기 있는 것만으로 기본 집계와 조인 질문 대부분을 풀 수 있습니다.**

예시는 모두 수업 원본 데이터(`bdai13-bigquery.tabformer`) 기준입니다. 원본 주소는 그대로 쓰고, 실행 프로젝트만 본인 프로젝트로 선택하세요.
...
| 문법 | 역할 | 예시 |
| --- | --- | --- |
| 백틱 3단 이름 | `프로젝트.데이터셋.테이블`을 백틱으로 감싼다 | `` FROM `bdai13-bigquery.tabformer.transactions` `` |
| `AS` 별칭 | 테이블에 짧은 이름을 붙인다 | `` FROM `...transactions` AS t `` |
| `t.컬럼` | 어느 테이블의 컬럼인지 밝힌다 | `t.user_id`, `u.gender` |

> 💡 **왜 별칭을 꼭 쓰는가.** 조인이 들어오는 순간 `user_id`가 어느 테이블 것인지 모호해집니다. 이 수업에서는 **테이블 별칭과 `t.컬럼` 표기를 항상 쓰는 것**을 규칙으로 합니다.
...
SELECT
    t.mcc,
    COUNT(*)   AS tx_cnt
FROM `bdai13-bigquery.tabformer.transactions` AS t
WHERE t.year = 2018
GROUP BY t.mcc
...
SELECT
    ...
    COUNT(*)   AS tx_cnt
FROM `bdai13-bigquery.tabformer.transactions` AS t
INNER JOIN `bdai13-bigquery.tabformer.users` AS u
...
`COUNTIF`는 BigQuery의 편의 함수입니다. 아래 두 줄은 같은 값을 내는데 위가 훨씬 짧습니다.

```sql
SELECT
    COUNTIF(t.is_fraud = "Yes")                        AS fraud_cnt,       -- BigQuery
    COUNT(CASE WHEN t.is_fraud = "Yes" THEN 1 END)     AS fraud_cnt_std    -- 다른 DB에서도 통하는 방식
FROM `bdai13-bigquery.tabformer.transactions` AS t;
```
...
채널별 사기율을 구하는 아래 쿼리에는 위 문법이 거의 다 들어 있습니다.
...
> 💡 **사기율이 아주 작게 나오는 것이 정상입니다.** 사기는 원래 희귀합니다. 원본 TabFormer 데이터의 전체 사기율은 약 0.12%이며, 수업 테이블의 값은 직접 실행해 확인하세요.
...
| --- | --- | --- |
| `SAFE_DIVIDE(x, y)` | 2주차 | 0으로 나누기 방어 |
| `WITH` (CTE) | 2~4주차 | 같은 정제를 반복해 쓰는 불편함 |
| 윈도우 함수 `OVER()` | 4주차 | 행을 남긴 채 순위나 이전 값을 옆에 쓰기 |
| `QUALIFY` | 4주차 (참고) | 윈도우 함수 결과 필터링 |
| `CREATE OR REPLACE VIEW` | 5주차 | 로직을 한 곳에만 정의 |
| `CREATE OR REPLACE TABLE ... AS` | 5주차 | 결과를 테이블로 저장 (데이터 마트) |
| `PARTITION BY` (테이블 설계) | 5주차 | 스캔 바이트와 비용 절감 |
