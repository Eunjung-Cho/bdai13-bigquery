...
| 5 | CREATE VIEW | 재사용할 쿼리 저장 | 원본 조회 비용은 계속 발생 가능 |
| 5 | CREATE TABLE AS | 계산 결과를 테이블로 저장 | 한 행의 기준, 갱신, 만료 관리 |
...
달러 기호를 제거하고, 천 단위 쉼표를 제거한 다음 숫자로 변환합니다. 소수, 음수는 유지합니다. `SAFE_CAST`는 입력값 변환 실패를 NULL로 처리하지만, 존재하지 않는 열이나 불가능한 타입 조합 같은 모든 오류를 해결하지는 않습니다.
...
카드 한 장이 한 줄씩만 저장돼 있다면 위 검사 결과는 0행이어야 합니다. 같은 고객 번호와 카드 번호가 여러 줄에 나오면 다른 열의 값도 비교하세요. 데이터를 두 번 넣었는지, 다른 이유가 있는지 확인하기 전에 `DISTINCT`로 지우지 않습니다.
...
SELECT 목록을 읽기 전에 원본과 결과의 한 행을 설명하면 JOIN과 집계 오류를 발견하기 쉽습니다.

## 오류 메시지별 첫 확인

| 메시지 또는 증상 | 첫 확인 | 수정 방향 |
| --- | --- | --- |
| Unrecognized name | Schema에 실제 열이 있는가? | 열 이름, 별칭 범위 확인 |
| No matching signature | 문자열을 숫자처럼 계산했는가? | 값이 숫자인지 글자인지 확인한 뒤 알맞게 변환 |
...
| Access Denied | 읽기와 실행 권한 중 어느 것이 없는가? | 오류가 난 프로젝트와 테이블 주소를 강사에게 전달 |
| Not found ... location | 데이터와 실행 위치가 같은가? | 같은 리전으로 설정 |
| 조인 후 금액 증가 | 오른쪽 키가 유일한가? | 키 검사와 복합 키 수정 |
| LAG 비교 기간이 건너뜀 | 월이 빠졌는가? | 달력 보완 후 LAG |
| 대시보드 평균이 SQL과 다름 | 평균을 다시 평균냈는가? | 분자와 분모 합으로 재계산 |

## BigQuery와 DuckDB를 구분하기

BigQuery와 DuckDB는 둘 다 SQL을 쓰지만 일부 명령의 이름과 쓰는 방법이 다릅니다. 예를 들어 BigQuery의 `SAFE_CAST`는 DuckDB에서 `TRY_CAST`로 대응할 수 있고, `SAFE_DIVIDE` 대신 `NULLIF(분모, 0)`로 나누는 수가 0일 때 NULL로 바꿀 수 있습니다. 완성된 BigQuery SQL을 DuckDB에 그대로 붙여 넣어 항상 실행된다고 가정하지 마세요. 예비 경로에서는 이미 정제된 마트를 가져와 간단한 SELECT, GROUP BY만 연습합니다.
