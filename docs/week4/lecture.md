...
> 2026년 10월 23일 금요일 20:00–22:00. 이번 주에는 전체 숫자를 월별로, 또 온라인과 오프라인 같은 그룹별로 나눠 봅니다. 변화가 나타난 위치를 찾고, AI의 요약이 표를 정확히 읽었는지 검증합니다.

[실습](lab.md) / [과제](assignment.md) / [프로젝트 안내](../project.md)

## 오늘의 목표와 시간표

- (1) 전체 변화와 그룹 변화를 구분합니다.
...
실습과 리뷰는 80분입니다. 3주차 조인 검사 결과와 [지표 정의서](../templates/metric-spec.md)를 준비합니다. 읽기 원본은 `bdai13-bigquery.tabformer`이며, 실제 공유 준비 상태는 [환경 준비](../setup.md)에서 확인합니다.
...
그룹 증감률의 평균은 전체 증감률이 아닙니다. 위 표의 전체 변화율은 `(270-300)/300 = -10%`입니다. +40%와 -35%의 단순 평균 2.5%를 쓰면 방향까지 반대로 나옵니다. 전체 비율은 전체 분자와 전체 분모로 다시 계산합니다.

## 2. GROUP BY는 행을 접고, 윈도우 함수는 옆에 값을 붙인다

`GROUP BY`는 같은 그룹의 여러 거래를 한 줄의 합계로 모읍니다. 윈도우 함수는 행을 그대로 두고 옆에 순위나 앞 행의 값을 붙입니다. 이번에는 먼저 연령대와 업종(MCC)이 같은 거래끼리 합칩니다. 그 결과의 각 연령대 안에서 순거래액 순서를 매깁니다. 거래 원본에 바로 순위를 붙이면 “가장 큰 거래”를 찾게 되므로 질문이 달라집니다.
...
달력의 빈칸을 0으로 채우려면 **그 기간의 데이터를 빠짐없이 모았고, 해당 그룹에서 실제 거래가 없었다는 점**을 먼저 확인해야 합니다. 수집 누락이나 정제 실패를 0으로 숨기지 않습니다. 먼저 전체 기간 프로파일과 실패 행 수를 확인하고, 원본 누락이면 변화율 해석을 보류합니다.
...
휴면 분류는 2018년 집계와 달리 기준일 **이전 전체 관측 이력**이 필요합니다. 2017년에 마지막으로 거래한 고객을 “거래 없음”으로 바꾸지 않기 위해서입니다. 지금의 고객 표가 2019년 당시 고객 명단과 같은지는 알 수 없습니다. 따라서 결과는 수업 규칙으로 분류한 후보이며, 실제로 그때 휴면 상태였던 고객 명단이라고 말하지 않습니다.
...
프롬프트에 “표에 없는 원인은 제안할 수 있으나 추정이라고 표시하고 필요한 추가 자료를 적으라”고 요청합니다. AI가 확신하는 말투로 써도, 결과표의 어느 칸에서 확인할 수 있는지 찾지 못했다면 그대로 사용하지 않습니다. 개인정보가 없는 집계표와 지표 정의만 전달합니다.

## 6. 개인 프로젝트를 시작한다

오늘부터 개인별로 질문 한 개를 고정하고 SQL과 검증 기록을 쌓습니다. 공동 발표를 준비하더라도 각자의 기여와 검증 과정을 추적할 수 있어야 합니다. 개인 작업과 팀 리뷰를 연결하는 세부 운영은 강사 안내를 확인합니다.

[프로젝트 문서](../project.md)에 질문, 분석 기간, 그룹, 지표 정의, 필요한 검증 두 개를 적습니다. “고객 행동 분석”보다는 “2018년 온라인 순거래액의 월별 변화가 어느 MCC에 집중되는가?”처럼 쿼리로 답할 수 있게 좁힙니다.
