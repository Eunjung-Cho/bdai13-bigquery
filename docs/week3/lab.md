...
읽기 원천은 `finda-13-2026.tabformer`이며 실제 공유 준비 상태는 [환경 준비](../setup.md)를 확인합니다. 각 SQL은 독립 실행용입니다. 실제 결과와 처리 바이트는 본인이 기록합니다. 키 검증을 먼저 끝내고 분석으로 이동합니다.

처음에는 **참고 SQL을 펼쳐 그대로 실행 → 결과 컬럼 확인 → 조건 한 곳 수정 → 결과 비교** 순서로 진행합니다. 긴 쿼리를 외울 필요는 없습니다. `WITH` 다음 이름들은 “정제한 표”, “기간을 고른 표”, “속성을 붙인 표”라고 읽으면 됩니다. 도전 문제는 기본 실행과 검증을 마친 뒤 선택합니다.
...
- (2) 아래 중복/NULL 검사에서 0이 아닌 결과가 있으면 분석을 멈추고 원천을 확인합니다.
- (3) 조인 보존 검사를 실행해 결과 한 행과 처리 바이트를 저장합니다.
...
<details markdown="1">
<summary>조인 보존 검사 참고 SQL</summary>
...
정상적인 거래 기준 LEFT JOIN에서는 차이 두 개가 0이어야 합니다. `rows_before=0`이면 통과 판정을 하지 말고 데이터 기간, 권한, 정제 조건을 확인합니다. `missing_*`가 0이 아니어도 LEFT의 보존 자체는 가능하므로 누락과 증식을 구분합니다. 이 수업에서 errors가 비어 있음을 승인으로 간주하는 것은 교육용 정의입니다.

## 실습 2. 어느 연령대와 성별에서 소비가 관측되는가? — 15분

**조건:** 2018년, 유효 금액, 교육용 승인 조건, 환불 포함. 한 행은 스냅샷 연령대 × 성별입니다. 고객 수는 해당 그룹에서 거래한 고유 고객 수입니다.
...
해석은 “스냅샷 연령대 A에서 관측된 순거래액이 크다”처럼 씁니다. 그룹의 고객 수가 크기 때문인지, 거래 빈도 때문인지, 거래당 금액 때문인지 다음 질문을 덧붙입니다. “A 연령대는 원래 소비 성향이 강하다”는 결론은 이 표로 입증되지 않습니다.

## 실습 3. 한 번도 쓰이지 않은 카드는 몇 장인가? — 10분

“한 번도”의 범위를 먼저 정합니다. 여기서는 **제공된 전체 관측 기간에서 날짜와 금액이 유효하고 교육용 승인 조건을 만족한 거래가 0건인 카드**입니다. 실제로 발급 이후 평생 사용하지 않은 카드라는 뜻이 아닙니다. 현재 cards 스냅샷의 카드 보유 기간도 알려져 있지 않습니다.
...
`used_cards`에서의 DISTINCT는 오류를 덮는 중복 제거가 아닙니다. “고객과 카드별 사용 여부 한 행”이라는 새 그레인을 만드는 명시적인 집합 연산입니다. 브랜드별 `used_cards + unused_cards = all_cards`여야 합니다. 전체 `all_cards` 합은 카드 원본 행 수와 같아야 합니다.
...
**조건:** 소득은 스냅샷 연간 소득이며 USD 기준입니다. 경계는 30,000, 60,000, 100,000입니다. 음수 및 변환 실패는 미상으로 분리합니다. 객단가는 고객별 평균의 평균이 아니라 거래 전체의 순거래액/건수입니다.
...
| 소득 미상이 많음 | yearly_income 원문 형식 | 미상→0 대체 없이 원천 프로파일 |
