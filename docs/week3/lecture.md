...
| 20:25–20:45 | 키 유일성과 조인 보존 검사 | 20분 |
| 20:45–20:50 | 휴식 | |
...
거래 테이블 한 행은 거래 기록 한 건입니다. 고객 테이블 한 행은 고객 스냅샷 한 명입니다. 카드 테이블 한 행은 특정 고객의 특정 카드 한 장입니다. 이 세 문장이 조인 조건을 결정합니다.
...
`card_index=0`은 전체 고객을 통틀어 한 장을 뜻하지 않습니다. 각 고객 안의 카드 번호이므로 고객 번호와 함께 써야 합니다. `(user, card_index)`가 복합 키입니다. 거래에는 고유 `transaction_id`가 제공되지 않으므로 없는 ID를 만들어 중복 제거의 근거로 쓰지 않습니다.
...
`INNER JOIN`은 양쪽 키가 일치한 행만 남깁니다. `LEFT JOIN`은 왼쪽 행을 남기고, 일치하는 오른쪽 행이 없으면 오른쪽 컬럼을 NULL로 채웁니다. 오른쪽에 같은 키가 여러 행이면 LEFT도 행을 늘릴 수 있습니다. [GoogleSQL 조인 문법](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/query-syntax#join_types)
...
| 거래가 늘었거나 줄었는가? | 동일 기간과 금액 조건에서 전후 COUNT | LEFT enrichment라면 행 수 동일 |
...
## 5. 세그먼트 이름에도 정의가 필요하다

`current_age`는 고객 파일의 스냅샷 나이입니다. 2018년 거래를 분석하더라도 “2018년 당시 나이”라고 쓰지 않습니다. 나이가 NULL 또는 음수이면 연령 미상으로 둡니다. 성별이 없으면 미상으로 표시합니다.
...
AI에 스키마, 기간, 금액 정제, 복합 키, 분석 그레인을 함께 제공합니다. 개인별 거래 행을 붙여 넣을 필요는 없습니다. [AI 검증 기록](../reference/ai-verification.md)에 생성문과 수정문을 남깁니다.
...
조인은 컬럼을 붙이는 작업이면서 분석 대상과 행 수를 바꾸는 작업입니다. 키 유일성, 행 수, 금액, 연결 실패를 함께 검사한 다음 해석합니다. 다음 주에는 이렇게 연결한 결과를 월과 세그먼트로 나눠 변화가 나타난 위치를 찾습니다.
