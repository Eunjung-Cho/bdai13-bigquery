...
하나의 비즈니스 질문에 대해 **정의 → SQL → 검증 → 마트 → Streamlit 대시보드**가 연결되는 결과물을 만듭니다. 기능이 많은 앱보다, 표시한 숫자의 근거를 직접 설명할 수 있는 작은 앱을 목표로 합니다.
...
| 채널별 사기 라벨 | 채널별 라벨 비율과 거래 규모는? | 분자·분모·미분류 라벨 | 실제 사기 탐지 성능으로 해석하지 않음 |
| 카드 이용 분석 | 보유 카드 중 관측 기간에 미사용인 카드는? | 복합 키·관측 기간·anti join | 평생 미사용이라고 단정하지 않음 |
| 고객 소비 구성 | 고객 속성별 주요 업종은? | 조인 보존·그룹 규모·현재 나이 한계 | 개인의 능력·성향으로 일반화하지 않음 |

기본 앱은 월간 업종·채널 마트를 사용합니다. 다른 주제를 고르면 마트의 한 행과 필요한 컬럼을 먼저 정하고, 동일한 검증 기준을 적용합니다.

## 회차별 진행

| 시점 | 할 일 | 보여줄 근거 |
| --- | --- | --- |
| 3주차 끝 | 주제 후보와 조인 경로 논의 | 질문 1문장·원천·키 |
| 4주차 | 지표와 첫 분석 결과 작성 | 지표 정의서·SQL·교차 검증 |
| 5주차 | 재사용할 마트와 앱 연결 준비 | CREATE SQL·집계 일치·권한 확인 |
| 6주차 | 앱 수정·공유·발표 | 앱 URL·숫자 대조표·한계 설명 |

원본 계획서에는 개인 최종 프로젝트와 팀 구성·팀 발표가 함께 포함되어 있습니다. **개인별 분석·AI 검증 기록을 남기고 팀이 공동 리뷰·발표하는 운영안**으로 준비하며, 팀 규모와 최종 제출 단위는 강사 공지를 따릅니다.
...
실제 결과를 아직 구하지 않았다면 가상 값으로 채우지 않습니다. 발표 때도 관찰된 결과와 가설을 분리해 말합니다.
...
| --- | --- | --- |
| 질문과 정의 | 기간·그레인·분모가 일치함 | 지표 이름만 있고 기준이 없음 |
