...
- [Colab에서 BigQuery→DuckDB 노트북 열기](https://colab.research.google.com/github/Eunjung-Cho/bdai13-bigquery/blob/main/docs/downloads/bigquery-duckdb.ipynb): 브라우저에서 바로 열어 셀을 차례대로 실행합니다. 5주차에 만든 `bdai13-bigquery.bdai13.mart_monthly_mcc`가 있어야 하며, 로그인한 Google 계정으로 그 표를 읽을 수 있어야 합니다.
...
먼저 [환경 준비](setup.md), 이후 [Streamlit과 데이터 연결](reference/dashboard-connection.md)을 읽으세요. 노트북의 기본 주소는 수업 공통 프로젝트 `bdai13-bigquery`와 서울 리전입니다. 팀별 별도 프로젝트를 쓰기로 정한 경우에만 실행 프로젝트, 마트 주소, 리전을 함께 바꿉니다. 파일을 수정한 뒤에는 지표 정의와 검증표도 함께 업데이트합니다.
