...
모든 실제 조회는 `finda-13-2026.tabformer`를 읽습니다. 스키마가 아래와 다르면 열을 추측해서 바꾸지 말고 [데이터 사전](../data-guide.md)과 실제 스키마를 대조합니다. 실행 프로젝트는 본인 프로젝트이며 원본과 같은 데이터 위치를 사용합니다.
...
첫 쿼리와 아래 쿼리의 예상 바이트를 실행 전에 비교합니다. 이미 예상치 비교로 답할 수 있다면 비용 비교를 위해 둘 다 반복 실행하지 않습니다. 테이블의 물리 구조와 캐시 여부에 따라 관찰값이 달라질 수 있으므로 값을 만들어 채우지 않습니다.
...
**한 부분 수정:** `COUNTIF(mcc IS NULL)`의 `mcc`만 `card_id`로 바꾸고 결과 열 이름도 `missing_card_rows`로 바꿉니다. 어떤 결측을 확인했는지 문장으로 적습니다.

### 필수 B2 · 금액·날짜를 해석할 수 있는가?
...
`REPLACE`는 `$`와 쉼표를 제거하고, `SAFE_CAST`는 숫자로 읽을 수 없는 값을 NULL로 둡니다. `FORMAT`은 연·월·일을 고정된 자리수로 조합하며, `SAFE.PARSE_DATE`는 유효하지 않은 날짜를 NULL로 둡니다. NULL을 0달러나 임의의 날짜로 대체하지 않습니다.

`unusable_amount_rows`에는 원본 NULL, 빈 문자열, 변환 불가 문자열이 함께 포함됩니다. “모두 변환 오류”라고 쓰지 말고 원본 결측과 구별합니다. 금액 정제의 상세 설명은 다음 주에 다시 다룹니다.
...
같은 `card_index`를 가진 사용자가 여럿 있다면 `card_index`만으로 카드를 식별할 수 없습니다. 그렇다고 해당 카드들을 지워서는 안 됩니다. 카드 키의 유일성은 `(user, card_index)` 조합으로 확인하며 3주차에 이어집니다.
