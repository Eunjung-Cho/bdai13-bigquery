...
- 행 수·날짜·결측·자료형을 점검하고 분석의 전제를 적습니다.
- AI가 만든 SQL을 스키마, 실행, 의미의 세 단계로 검증합니다.
...
| 21:30–21:55 | C. AI SQL 생성·검증·수정 | 실습 25분 |
...
이를 답하려면 날짜와 금액을 읽을 수 있어야 하고, MCC가 무엇인지 알아야 합니다. **오늘은 순위를 만들기 전에 재료를 확인합니다.** 다음 주에는 지표를 정의하고 집계하며, 이후 고객·카드를 연결하고 기간별 변화와 세그먼트를 분석합니다. 마지막에는 반복 조회를 위한 데이터 마트와 대시보드를 만듭니다.
...
아닐 수 있습니다. 온디맨드 쿼리에서는 읽는 데이터의 바이트가 비용과 관련됩니다. 일반 비클러스터 테이블의 `LIMIT 10`은 화면에 돌려주는 행 수를 제한하지만 스캔 비용을 줄이는 장치가 아닙니다. 테이블 미리보기로 값을 먼저 살피고, 필요한 열만 선택한 뒤 예상 처리 바이트를 확인합니다. [비용 추정과 통제](https://docs.cloud.google.com/bigquery/docs/best-practices-costs)
...
| 실행 후 | 처리 바이트·청구 바이트·캐시 여부 | 실제 작업 정보 확인 |
...
|---|---|---|---|
| `users` | 사용자 한 명의 속성 | `user_id` | `current_age`는 스냅샷 나이 |
| `cards` | 사용자가 가진 카드 하나 | `(user, card_index)` | 카드 번호만으로 전역 유일하지 않음 |
...
ERD는 확인할 관계를 표현합니다. `users.user_id`와 카드 복합 키가 실제로 유일한지는 프로파일링 결과로 확인해야 합니다. 조인은 3주차에 다루며, 오늘은 키를 읽고 기록하는 데 집중합니다.
...
프로파일링은 행 수, 고유값 수, 자료형, 범위, 결측 등을 요약하는 작업입니다. 숫자를 많이 뽑는 것이 목적은 아닙니다. “금액 변환 실패가 있는가?”, “분석하려는 기간이 실제로 존재하는가?”, “고객과 카드 키를 믿을 수 있는가?”를 답해야 합니다.
...
예상 결과는 전체 3행, NULL이 아닌 값 2개, NULL 또는 빈 문자열 2개입니다. `COUNT(amount)`가 정상적인 금액 개수는 아니라는 사실을 확인합니다. 실데이터 결과와 이 손계산 예시를 섞어서 기록하지 않습니다.

## 6. LLM은 왜 SQL을 만들 수 있고 왜 틀릴까?

LLM은 토큰이라는 텍스트 조각의 이어질 가능성을 학습해 문장을 생성합니다. 질문과 스키마도 입력 문맥에 들어갑니다. SQL의 반복되는 형태를 학습했기 때문에 질문에 맞는 쿼리를 제안할 수 있지만, 문장이 그럴듯하다는 사실이 데이터 정의의 정확성을 보장하지는 않습니다. [Google의 언어 모델 기초](https://developers.google.com/machine-learning/crash-course/llm)
