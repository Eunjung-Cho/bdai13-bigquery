...
```mermaid
flowchart LR
    R[2400만 건 원본] --> Q[BigQuery SQL로 정제와 집계]
    Q --> M[작은 월간 마트]
    M --> P[Python으로 조회]
    P --> S[Streamlit 필터, 차트]
```

BigQuery는 큰 데이터를 집계합니다. Python 코드는 그 작은 결과를 받아 화면에 맞게 전달하고, Streamlit은 필터, 지표, 차트를 표시합니다. 앱을 만든다는 이유로 원본 거래를 전부 다운로드하지 않습니다.

기본 경로는 BigQuery 직접 연결입니다. 인증 정책상 어려우면 Colab에서 집계 결과를 DuckDB 파일로 만들어 같은 앱이 읽게 합니다. DuckDB 파일은 사진처럼 저장할 때의 결과를 담고 있습니다. 이를 스냅샷이라고 하며, 새 결과를 보려면 파일을 다시 만들어야 합니다. [연결 안내](../reference/dashboard-connection.md)를 보고 자신의 경로와 갱신 방식을 설명할 수 있어야 합니다.
...
월, 채널, 업종 필터를 바꾸면 포함되는 마트 행이 달라집니다. 같은 조건을 SQL에도 적용해야 비교할 수 있습니다. 월간 마트에서 2018-03-15부터 2018-03-20까지처럼 일별 범위를 분석할 수는 없습니다. 원본보다 작아진 대신 상세 수준이 줄었습니다.

**연습용 예시:** A그룹은 2건에 총 100달러, B그룹은 3건에 총 900달러를 썼다고 해 봅시다. 전체는 5건에 1,000달러이므로 거래당 평균은 200달러입니다. 그룹별 평균인 50달러와 300달러를 다시 평균내면 175달러가 되어 답이 달라집니다. 앱에서 필터를 바꿀 때도 먼저 금액과 건수를 각각 더한 뒤 나눠야 합니다.
...
없는 열, 거래 결과, 활성 고객 수를 만들지 마.
객단가와 사기율은 분자와 분모를 합산한 뒤 계산해 줘.
```
...
우리가 Google 계정으로 로그인하듯, 앱도 데이터를 읽을 계정이 필요합니다. 앱이 사용하는 프로그램용 계정이 ‘서비스 계정’입니다. 앱은 서비스 계정의 권한으로 마트를 읽고, 방문자는 앱에서 공개한 집계값을 볼 수 있습니다. 키는 Streamlit Secrets에 넣고 공개 저장소에 올리지 않습니다. 큰 권한으로 문제를 해결하기보다 필요한 테이블의 조회 권한과 실행 프로젝트 권한을 확인합니다.

무료 Community Cloud도 자원, 휴면 조건이 있습니다. BigQuery 무료 한도는 쿼리 실행 프로젝트에 적용됩니다. 캐시는 한 번 가져온 결과를 잠시 보관했다가 다시 쓰는 기능입니다. `cache_data`를 쓰면 같은 조회를 줄일 수 있지만 무료 한도가 늘어나는 것은 아닙니다. 처음 열 때의 앱 준비 시간, 자료 취득 시각, 갱신 주기를 설명하세요.
...
사례 B: “한 연령대의 사기 라벨 비율이 높으니 그 연령대를 자동 차단하자.” 관측된 차이는 비교한 그룹의 크기나 주로 쓰는 채널, 합성 데이터를 만든 방식 때문에 나타났을 수 있습니다. 실제 차별적 조치를 정당화하는 근거가 아닙니다. 관찰과 정책 결정을 연결할 때 필요한 추가 검증을 토론합니다.
