...
2. `app.py`는 화면을 그리는 코드, `data_access.py`는 데이터를 가져오는 코드입니다. `requirements.txt`는 앱 실행에 필요한 도구(패키지)의 목록입니다.
...
월 드롭다운에서 `2018-03`을 고르고 채널 드롭다운에서 `온라인`을 고릅니다. 지표 카드와 두 차트가 같은 조건을 따르는지 봅니다. 이어서 월과 채널을 다시 `전체`로 바꾸고 2018년 전체 숫자로 돌아오는지 확인합니다.

필터는 이미 제공되어 있습니다. 선택 도전으로 AI에게 기본 선택 월만 바꿔 달라고 요청할 수 있습니다. 필터를 조작할 때마다 원본 전체를 다시 조회하게 바꾸지 않습니다.
...
아래 쿼리는 마트에 저장된 여러 줄을 다시 더해 앱과 같은 숫자를 구합니다. 앱과 같은 마트 주소로 바꾸고 실행합니다.
...
| 사기율이 SQL 결과와 다름 | 사기 여부를 아는 건수로 나눴는지, 반올림 차이인지, 나누는 수가 0인지 확인 |
