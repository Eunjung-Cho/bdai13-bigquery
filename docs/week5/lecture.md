...
지난주까지는 질문마다 SQL을 작성했습니다. 이번 주에는 매번 반복하는 정리 작업을 저장하고, 자주 보는 합계를 작은 표로 만들어 둡니다. 이렇게 특정 분석에 쓸 데이터를 모아 둔 표를 **데이터 마트**라고 합니다. 목표는 SQL의 줄 수를 줄이는 데 그치지 않습니다. 다른 사람이 실행해도 같은 지표가 나오고, 비용과 갱신 시점을 설명할 수 있어야 합니다.

수업을 마치면 다음을 할 수 있습니다.

- 뷰와 저장 테이블의 차이를 데이터 갱신과 처리 바이트 관점에서 설명한다.
- 한 줄에 무엇을 담을지 먼저 정하고 월간 소비 마트를 만듭니다.
- 파티션과 클러스터가 도움이 되는 조회 조건을 구별한다.
- 원본, 뷰, 마트의 결과가 같은지 검증한 뒤 처리 바이트를 비교한다.
- AI의 코드 정리 제안을 검증 기록과 함께 채택하거나 거절한다.

## 120분의 흐름

| 시간 | 활동 | 직접 실습 |
| --- | --- | --- |
| 20:00–20:12 | 반복 보고 문제, OLTP, OLAP, 한 행의 기준 | — |
...
| 21:32–21:47 | 실습 D: AI와 코드 정리, 틀릴 수 있는 예시 확인 | 15분 |
| 21:47–22:00 | 결과 해석, 프로젝트 마트 설계 리뷰 | — |

직접 실습은 80분입니다. 선택 Silver 테이블 실습은 진행 속도가 빠른 수강생 또는 과제 확장용입니다.
...
| 주로 다루는 범위 | 특정 거래 또는 고객 | 기간과 여러 그룹 |
| 중요한 특성 | 각 기록이 정확한지 확인하고 빠르게 수정 | 집계 정의의 일관성, 읽기 효율 |
| 수업에서의 위치 | 원본 업무를 이해하는 배경 | BigQuery에서 수행하는 집계 |
...
표의 한 줄에 거래 한 건을 담을지, 한 달의 합계를 담을지 먼저 정합니다. 이런 ‘한 행의 기준’을 전문 용어로는 grain이라고 합니다. 열 이름부터 나열하기보다 ‘한 줄은 무엇이다’라고 말해 보세요.
...
| 열 | 의미 | 다시 합쳐도 되는가? |
| --- | --- | --- |
| `tx_month` | 거래일을 해당 월의 1일로 변환한 DATE | 그룹을 나누는 기준, 합산하지 않음 |
| `mcc` | 업종 식별 코드 | 그룹을 나누는 기준, 합산하지 않음 |
| `channel` | 온라인, 오프라인, 미분류 | 그룹을 나누는 기준, 합산하지 않음 |
...
`txn_count`는 마트의 행 수가 아닙니다. 마트에 온라인과 오프라인 두 행이 있어도 그 안에는 여러 건의 거래가 들어 있습니다. 대시보드의 `Record Count`로 거래 건수를 대체하면 집계 행 개수를 세는 오류가 생깁니다.
...
가운데에는 거래 기록과 금액이 담긴 표를 두고, 주변에는 고객, 카드, 업종 정보를 담은 표를 둔다고 생각해 보세요. 가운데의 거래 표를 **팩트**, 분석을 나누는 기준이 되는 주변 정보를 **차원**이라고 부릅니다. 이 연결 그림이 별 모양과 비슷해서 **스타 스키마**라고 합니다.
...
원본에는 고유 `transaction_id`가 없습니다. 임의 열 하나를 거래 키로 간주하거나 `DISTINCT`로 겉보기 중복을 없애지 않습니다. MCC 설명표도 원본 세 테이블에 포함되어 있다고 가정하지 않습니다. 별도 코드표를 붙인다면 같은 코드가 여러 줄에 있지 않은지, 어느 시점의 설명표인지 확인해야 합니다.

이번 마트에는 질문에 필요한 월, 업종, 채널만 담습니다. 연령별로도 보고 싶다면 결과 한 줄을 어떻게 나눌지 다시 정하고 고객 정보를 붙인 뒤 숫자가 맞는지 확인해야 합니다. 이때 `current_age`는 거래 당시 나이가 아니라 고객 정보를 저장할 때의 나이라는 점을 적습니다.

## 4. 뷰와 CTAS는 무엇을 저장하는가?

논리 뷰는 자주 쓰는 SQL에 이름을 붙여 저장한 것입니다. 요리로 비유하면 만드는 순서를 적어 둔 레시피와 비슷합니다. `SELECT ... FROM 뷰`를 실행하면 그 정의에 따라 원본을 조회합니다. 복잡한 정제식을 공통으로 쓰기에 좋지만 결과 테이블을 미리 저장한 것은 아닙니다.

CTAS는 `CREATE TABLE ... AS SELECT ...`의 줄임말입니다. 레시피대로 만든 음식을 담아 두듯, SELECT로 계산한 결과 자체를 새 표에 저장합니다. 나중에 조회할 때는 저장된 결과를 읽지만, 원본이 바뀌었다고 결과가 자동 갱신되지는 않습니다.

| 구분 | 정제 뷰 | 집계 테이블 |
| --- | --- | --- |
| 저장하는 것 | 쿼리 정의 | 실행 시점의 결과 |
| 쓰임 | 정제식과 열 의미 통일 | 반복되는 집계를 미리 계산 |
...
날짜가 불가능한 조합이면 NULL로 남깁니다. 월별 집계는 이 정제 날짜를 `DATE_TRUNC(tx_date, MONTH)`로 월초에 맞춰 묶습니다. 정제 뷰에는 전체 기간을 남기고, 2018년 분석 범위와 유효 금액 조건은 마트를 만드는 단계에서 분명하게 적습니다.

뷰 생성 전체 SQL과 생성 뒤 품질 검사는 [실습 A](lab.md)에 있습니다. 쓰기는 자신의 `YOUR_PROJECT.bdai13`에서 하고, 강사 공유 원본인 `bdai13-bigquery.tabformer`는 읽기 대상으로만 사용합니다.

## 5. 파티션과 클러스터: 필요한 범위를 읽게 하기

파티션은 큰 표를 날짜 같은 기준으로 나눠 저장한 구간입니다. 서류를 월별 서랍에 나눠 두면 10월 서류를 찾을 때 다른 달의 서랍을 열 필요가 없는 것과 비슷합니다. SQL에서 파티션에 사용한 날짜 열의 범위를 지정하면 필요한 구간을 골라 읽을 수 있습니다. 오늘은 월 단위로 나눕니다.

클러스터는 저장할 때 같은 업종처럼 비슷한 값을 가까이 모아 두는 방식입니다. 서랍 안에서 비슷한 서류끼리 정리해 두는 것으로 생각하면 됩니다. 예를 들어 `CLUSTER BY mcc, channel`인 테이블에서 특정 MCC를 자주 조회하면 읽을 블록을 줄일 가능성이 있습니다. 열 순서도 중요합니다.
...
1. 원본에서 정제하고 월별 집계하기.
2. 정제 뷰에서 같은 월별 집계하기.
3. 월간 마트에 저장한 금액과 거래 건수를 월별로 다시 더하기.

기간, 승인 정의, 환불, NULL 처리, 출력 열이 같아야 합니다. 더 적은 열과 더 짧은 기간을 읽는 다른 질문으로 바뀌었다면 공정한 비교가 아닙니다.
...
`LIMIT 10`은 출력 행을 줄이는 구문입니다. 일반적인 비클러스터 테이블에서 읽을 열의 전체 스캔을 줄인다고 보장하지 않습니다. `SELECT *`를 필요한 열 목록으로 바꾸는 제안과 같은 효과로 취급하지 않습니다.
...
```text
다음 BigQuery GoogleSQL 코드를 읽기 쉽게 정리해 주세요.
출력 한 행의 기준은 월 × MCC × 채널입니다.
...
다음 주에는 월, 업종, 채널이라는 세 가지 분류 기준과 순거래액, 거래 건수, 사기 건수, 라벨 확인 건수라는 네 가지 숫자를 앱에 연결합니다. 마트에 없는 고객별 상세, 일별 추이, 월 전체 고유 고객 수는 차트 설정만으로 복구할 수 없습니다.

마트와 함께 아래 정보를 전달합니다.

- 질문과 한 행의 기준, 분석 기간, 승인, 환불, 사기율 분모 정의.
