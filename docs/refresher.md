...
약 40분 동안 작은 표를 보며 SQL의 네 가지 기본 동작을 연습합니다. SELECT는 열 고르기, WHERE는 조건에 맞는 행 고르기, GROUP BY는 같은 항목끼리 묶어 계산하기, JOIN은 두 표 연결하기입니다. 아래 표는 **직접 만든 교육용 가상 데이터**이며 실제 TabFormer 분석 결과가 아닙니다. 모든 예제는 외부 테이블 권한 없이 BigQuery에서 실행할 수 있습니다.
...
가상 예시의 `tx_id`는 설명용입니다. 수업 원본 데이터에 같은 열이 있다고 가정하지 마세요. 1월의 순거래액은 손으로 `100 - 20 + 50 = 130`달러라고 계산할 수 있습니다.
...
전체 객단가는 `(80 + 50) / (2 + 1) = 약 43.33`달러입니다. 고객별 객단가의 평균 `(40 + 50) / 2 = 45`는 다른 지표입니다. 이 구분은 6주차 대시보드에서도 반복됩니다.
...
고객 3은 거래가 없지만 LEFT JOIN이므로 결과에 남습니다. 거래가 없어도 고객 3을 표시하기 위한 한 줄은 남습니다. `COUNT(*)`는 이 줄도 세기 때문에 거래가 1건인 것처럼 보입니다. `COUNT(t.user_id)`는 오른쪽에서 실제로 연결된 거래만 셉니다. 금액 합은 NULL이며, 보고 목적에 따라 “거래 없음”을 확인한 뒤 0으로 표시할 수 있습니다.
...
위 순서는 SQL의 뜻을 읽는 순서입니다. BigQuery가 실제로 계산하는 순서는 더 빠르게 처리하기 위해 달라질 수 있습니다. WHERE는 더하기 전에 어떤 행을 남길지 정하고, HAVING은 더한 결과를 보고 어떤 그룹을 남길지 정합니다.
...
없는 열을 만들지 말고, 가정과 NULL 처리 방식을 설명해 줘.
```
