...
실습 70분입니다. 완성 SQL을 먼저 실행하고 조건 한 곳을 바꾸는 방식으로 진행합니다. 아래 코드의 원천 프로젝트는 `finda-13-2026`이며 개설·공유 완료는 강사 공지로 확인합니다. 모든 결과 수치는 직접 실행해서 기록합니다.

## A. 어느 업종의 순거래액이 큰가 · 25분

2018년 교육용 승인 조건, 유효 날짜·금액, 환불 포함이라는 정의를 적용합니다. 한 행은 MCC 업종 코드입니다. 알려진 업종명 매핑이 없으면 AI에게 이름을 추측하게 하지 않습니다.

1. 지표 정의서에서 기간·환불·분모를 확인합니다.
2. 아래 SQL을 실행하고 출력 컬럼을 읽습니다.
...
날짜가 아닌 월 번호만 쓰면 여러 연도 자료를 분석할 때 서로 다른 해의 같은 월이 섞일 수 있습니다. `DATE_TRUNC`는 연도를 보존한 월초 날짜를 만듭니다. 어떤 달이 없으면 0건이라고 바로 확정하지 말고 원천 기간과 적재 범위를 확인합니다. 월 달력 보완은 4주차에 배웁니다.
...
사기 라벨이 확인된 거래만 분모로 씁니다. 금액·기간·승인 조건은 앞과 같고, NULL 라벨은 전체 거래 건수에는 남지만 사기율 분모에서는 제외합니다.
...
**교육용 잘못된 식:** `AVG(채널별 사기율)`은 각 채널의 라벨 수가 다른데 같은 가중치로 평균냅니다. 강의의 가상 표에서 전체 1/3과 두 채널 평균 1/4가 왜 다른지 손으로 확인하세요.
...
| DATE 함수 오류 | 안전한 날짜 파싱과 원본 연·월·일 |
